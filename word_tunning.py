...
n_input = n_class = 27 # 마지막 글자를 분류 가능한 수. 입력 가능한 글자 
n_stage = 3
...
def make_batch(seq_data):
    ...
    for seq in seq_data:
        ...
        input = [num_dic[n] for n in seq[0:3]] # 앞의 문자 3개를 num_dic 전환
        #target = num_dic[seq[3]] #마지막 글자를 num_dic 전환
        target = num_dic[seq[-1]] #마지막 글자를 num_dic 전환
        input_batch.append(np.eye(27)[input]) # one hot encoding 첫 3개 문자 된다.
...
# test data 구성. 4자를 맞추기 위해, 4번째는 의미없는 알파벳을 사용해서 넣어준다.
#seq_data = ['bod-', 'bad-', 'boe-', 'nar-', 'nef-', 'zzz-']
# make_batch함수의 index를 조정해서 3개의 문자도 될 수 있도록 할 수 있다.
seq_data = ['bod', 'bad', 'boe', 'nar', 'nef', 'zzz']
# RNN model
#prediction = tf.cast(tf.argmax(model, 1), tf.int32)
input_batch, target_batch = make_batch(seq_data) # 단어 10개 encoding
